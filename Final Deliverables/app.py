...
def predict():
    if request.method=='POST':
        ...
        payload_scoring = {"input_data": [{"fields": ['GRE Score', 'TOEFL Score', 'University Rating', 'SOP', 'LOR', 'CGPA', 'Research'], "values": [arr]}]}

        response_scoring = requests.post('https://us-south.ml.cloud.ibm.com/ml/v4/deployments/10a9a42d-0d6b-42b2-8e89-d88ce774e56b/predictions?version=2022-11-18', json=payload_scoring,
        headers={'Authorization': 'Bearer ' + mltoken}).json()

        result=response_scoring['predictions'][0]['values']

        value=round(result[0][0]*100,2)
        if value>70:
